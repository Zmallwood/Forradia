...
_NS_START_
void creature::init() { movem_spd_ *= (rand_int(3) + 1) / 2.0f; }
...
void tree_object::init(str_view obj_type_name)
{
    ...
    for (auto i = 0; i < num_trunk_parts; i++)
    {
        ...
        needle_types_.push_back(
            needles_type ? hash(needles_name + std::to_string(needles_type))
                         : 0);
    }
}

void objects_stack::clear_objs() { objects_.clear(); }
...
int objects_stack::get_sz() const { return objects_.size(); }

void tile::init()
{
    objects_stack_ = std::make_shared<Forradia::objects_stack>();
}

void tile::set_ground(str_view ground_name) { ground_ = hash(ground_name); }
...
{
    curr_w_area_ = std::make_shared<world_area>(w_area_sz, world_scaling);
}
_NS_END_