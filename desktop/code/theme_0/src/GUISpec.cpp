    ...
    namespace SpecializedGUI
    {
        ...
        void GUIInventoryWindow::RenderDerived() const
        {
            ...
            auto marginY{ConvertWidthToHeight(
                k_margin, _<Engine::SDLDevice>().GetWindow())};
            ...
            auto numRows{
                CInt((bounds.height - 2 * marginY - (yStart - bounds.y)) /
                     slotHeight)};
            ...
            for (auto y = 0; y < numRows; y++)
            {
                for (auto x = 0; x < numColumns; x++)
                {
                    _<Engine::Renderers::Image2DRenderer>().DrawImage(
                        ...
                        yStart + y * (slotHeight + marginY), slotWidth,
                        slotHeight);
                }
            }
        }
        ...
        void GUIPlayerBodyWindow::Initialize()
        {
            ...
            auto leftArmBodyImageButton{std::make_shared<
                GUIComponentsLibrary::GUIButton>(
                0.1f - imageWidth / 2 + imageWidth, 0.04f, imageWidth,
                imageHeight, "",
                [this]
            ...
            auto legsImageButton{
                std::make_shared<GUIComponentsLibrary::GUIButton>(
                    0.1f - imageWidth / 2, 0.04f + imageHeight, imageWidth,
                    imageHeight, "",
                    [this]
                    {
                        SelectBodyPart(CInt(
            ...
            m_labelBodyPartTemperature =
                std::make_shared<GUIComponentsLibrary::GUILabel>(
                    0.01f + 0.01f, 0.3f + 3 * 0.02f, 0.2f, 0.05f,
                    "Temperature: ");
        ...
        {
            auto &playerBody{_<Theme0::GameplayCore::Player::PlayerCharacter>()
                                 .GetBodyRef()};
        ...
        void GUIInteractionMenu::UpdateDerived()
        {
            ...
            {
                auto menuEntryRect{
                    RectF{bounds.x + 0.01f + k_indentWidth,
