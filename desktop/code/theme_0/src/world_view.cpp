        ...
        void world_view::render() const
        {
            ...
            static func<bool()> fn_draw_regular_ground{
                [&]
                {
                             ...
                             tl_sw->elev() < tl_se->elev())
                    {
                        h_canv += tl_sz.h / 2;
                        full_ground_name =
                            ground_name + "_slope_south_west_inverted";
                    }
                    else if (tl_se->elev() < tl_nw->elev() &&
                             tl_se->elev() < tl_ne->elev() &&
                             tl_se->elev() < tl_sw->elev())
                    {
                        h_canv += tl_sz.h / 2;
                        full_ground_name =
                            ground_name + "_slope_south_east_inverted";
                    }
                    else if (tl_nw->elev() < tl_ne->elev() &&
                             tl_nw->elev() < tl_sw->elev() &&
                             tl_nw->elev() < tl_se->elev())
                    {
                        y_canv -= tl_sz.h / 2;
                        full_ground_name =
                            ground_name + "_slope_north_west_inverted";
                    }
                    else if (tl_ne->elev() < tl_nw->elev() &&
                             tl_ne->elev() < tl_sw->elev() &&
                             tl_ne->elev() < tl_se->elev())
                    {
                        full_ground_name =
                            ground_name + "_slope_north_east_inverted";
                    }
                             ...
                             tl_se->elev() < tl_sw->elev())
                    {
                        ...
                        full_ground_name =
                            ground_name +
                            "_slope_diagonal_south_west_to_north_east";
                    }
                             ...
                             tl_sw->elev() < tl_nw->elev())
                    {
                        ...
                        full_ground_name =
                            ground_name +
                            "_slope_diagonal_north_west_to_south_east";
                    }
                    ...
                    for (auto j = 0; j < 2; j++)
                    {
                        ...
                        case dirs::n:
                            for (auto k = 0.0f; k < part_len; k += 0.5f)
                            {
                                ...
                                _<engine::Renderers::img_2d_rend>().draw_img(
                                    "river_part_" + std::to_string(anim_idx),
                                    b.x, b.y, b.w, b.h);
                            }
                            break;
                        case dirs::e:
                        ...
                        case dirs::ne:
                            ...
                            {
                                rect_f b{
                                    x_canv + w_canv / 2 - river_part_w / 2 +
                                        k * river_part_w,
                                    y_canv + h_canv / 2 - river_part_h / 2 -
                                        k * river_part_h,
                                    river_part_w, river_part_h};
            ...
            static func<void()> fn_draw_water{
                []
                {
                    auto anim_idx{(ticks() + ((x_coord + y_coord) * 100)) /
                                  500 % 3};
                    _<engine::Renderers::img_2d_rend>().draw_img(
                        "ground_water_" + std::to_string(anim_idx), x_canv,
                        y_canv, w_canv, h_canv);

                    for (auto i = 0; i < tl->elev(); i++)
                        _<engine::Renderers::img_2d_rend>().draw_img(
                            "ground_water_height", x_canv, y_canv, w_canv,
                            h_canv);
                    for (auto i = 0; i < tl->water_depth(); i++)
                        _<engine::Renderers::img_2d_rend>().draw_img(
                            "ground_water_depth", x_canv, y_canv, w_canv,
                            h_canv);
                }};
            static func<void()> fn_draw_layers{
                []
                {
                    if (ground == hash("ground_grass"))
                        _<engine::Renderers::img_2d_rend>().draw_img(
                            "ground_grass_layer", x_canv, y_canv, w_canv,
                            h_canv);
                    else if (ground == hash("ground_dirt"))
                        _<engine::Renderers::img_2d_rend>().draw_img(
                            "ground_dirt_layer", x_canv, y_canv, w_canv,
                            h_canv);
                    else if (ground == hash("ground_rock"))
                        _<engine::Renderers::img_2d_rend>().draw_img(
                            "ground_rock_layer", x_canv, y_canv, w_canv,
                            h_canv);
                }};
            static func<void()> fn_draw_tile_symbols{
                []
                {
                    ...
                    if (x_coord == hovered_coord.x &&
                        y_coord == hovered_coord.y)
                        _<engine::Renderers::img_2d_rend>().draw_img(
                ...
                [&]
                {
                    ...
                    if (objs_stack)
                    {
                        for (const auto &obj : objs_stack->objects())
                        {
                            ...
                            {
                                auto tree_obj{std::static_pointer_cast<
                                    Forradia::tree_object>(obj)};
                                ...
                                for (auto i = 0; i < trunk_parts.size(); i++)
                                {
                                    ...
                                    _<engine::Renderers::img_2d_rend>()
                                        .draw_img(trunk_part_name,
                                                  trunk_part_x_center -
                                                      trunk_part_width / 2,
                                                  trunk_part_y_center -
                                                      trunk_part_height / 2,
