            ...
            void WorldView::Render() const
            {
                ...
                for (auto y = -extraRows; y < gridSize.height + extraRows; y++)
                {
                    for (auto x = 0; x < gridSize.width; x++)
                    {
                        ...
                        auto coordinateSEE{
                            Point{xCoordinate + 2, yCoordinate + 1}};
                        auto coordinateSESE{
                            Point{xCoordinate + 2, yCoordinate + 2}};
                        auto coordinateSES{
                            Point{xCoordinate + 1, yCoordinate + 2}};
                        ...
                        auto elevationSW{tileSW ? tileSW->GetElevation()
                                                : 0.0f};
                        auto elevationNEE{tileNEE ? tileNEE->GetElevation()
                                                  : 0.0f};
                        auto elevationSEE{tileSEE ? tileSEE->GetElevation()
                                                  : 0.0f};
                        auto elevationSESE{tileSESE ? tileSESE->GetElevation()
                                                    : 0.0f};
                        auto elevationSES{tileSES ? tileSES->GetElevation()
                                                  : 0.0f};
                        ...
                        for (auto object : objects)
                        {
                            ...
                            {
                                auto treeObjects{std::static_pointer_cast<
                                    Forradia::TreeObject>(object)};
                        ...
                        _<Engine::Renderers::GroundRenderer>().DrawTile(
                            ground, xCoordinate - 1, yCoordinate - 1,
                            rendTileSize, cameraPos, elevations, elevHeight);
