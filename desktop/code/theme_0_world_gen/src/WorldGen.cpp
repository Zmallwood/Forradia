        ...
        namespace WorldGeneration
        {
            ...
            void WorldGenerator::GenerateGrass() const
            {
                ...
                for (auto i = 0; i < numGrassAreas; i++)
                {
                    ...
                    {
                        for (auto x = xCenter - radius; x <= xCenter + radius;
                             x++)
                        {
            ...
            void WorldGenerator::GenerateSingleLake(int minX, int minY,
                                                    int maxX, int maxY,
                                                    int recursion) const
            {
                if (recursion == 0)
                        ...
                        for (auto x = xCenter - r; x <= xCenter + r; x++)
                        {
                            ...
                            if (dX * dX + dY * dY <= r * r)
                            {
                                ...
                                if (tile)
                                {
                                    ...
                                    auto elevationN{
                                        tileN ? tileN->GetElevation() : 0};
                                    auto elevationE{
                                        tileE ? tileE->GetElevation() : 0};
                                    auto elevationS{
                                        tileS ? tileS->GetElevation() : 0};
                                    auto elevationW{
                                        tileW ? tileW->GetElevation() : 0};
                                    auto elevationNW{
                                        tileNW ? tileNW->GetElevation() : 0};
                                    auto elevationNE{
                                        tileNE ? tileNE->GetElevation() : 0};
                                    auto elevationSE{
                                        tileSE ? tileSE->GetElevation() : 0};
                                    auto elevationSW{
                                        tileSW ? tileSW->GetElevation() : 0};
                                    auto elevationNN{
                                        tileNN ? tileNN->GetElevation() : 0};
                                    auto elevationWW{
                                        tileWW ? tileWW->GetElevation() : 0};
                                    auto elevationEE{
                                        tileEE ? tileEE->GetElevation() : 0};
                                    auto elevationSS{
                                        tileSS ? tileSS->GetElevation() : 0};
                                    auto elevationNWNW{
                                        tileNWNW ? tileNWNW->GetElevation()
                                                 : 0};
                                    auto elevationNENE{
                                        tileNENE ? tileNENE->GetElevation()
                                                 : 0};
                                    auto elevationSESE{
                                        tileSESE ? tileSESE->GetElevation()
                                                 : 0};
                                    auto elevationSWSW{
                                        tileSWSW ? tileSWSW->GetElevation()
                                                 : 0};
                                    ...
                                    tile->SetWaterDepth(tile->GetWaterDepth() +
                                                        1);
                                }
                            }
                        }
                        ...
                        for (auto y = yCenter - r; y <= yCenter + r; y++)
                        {
                            for (auto x = xCenter - r; x <= xCenter + r; x++)
                            {
                                ...
                                if (dX * dX + dY * dY <= r * r)
                                {
                                    ...
                                    {
                                        auto tileN{
                                            m_worldArea->GetTile(x, y - 1)};
                                        auto tileS{
                                            m_worldArea->GetTile(x, y + 1)};
                                        auto tileW{
                                            m_worldArea->GetTile(x - 1, y)};
                                        auto tileE{
                                            m_worldArea->GetTile(x + 1, y)};
                                        auto tileNW{
            ...
            void WorldGenerator::GenerateRivers() const
            {
                ...
                for (auto i = 0; i < numRivers; i++)
                {
                    ...
                    for (auto j = 0; j < length; j++)
                    {
                        ...
                        auto tile =
                            m_worldArea->GetTile(xCoordinate, yCoordinate);
            ...
            void WorldGenerator::GenerateNPCs() const
            {
                ...
                for (auto i = 0; i < numNPC0s; i++)
                {
                    ...
                    {
                        auto newNPC =
                            std::make_shared<Theme0::WorldStructure::NPC>(
                                "npc0");
