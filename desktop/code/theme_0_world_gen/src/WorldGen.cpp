...
namespace Forradia
{
    namespace Theme0
    {
        namespace WorldGeneration
        {
            ...
            {
                m_worldArea =
                    _<Theme0::WorldStructure::World>()
                        .GetCurrentWorldArea();

                m_size = m_worldArea->GetSize();

                m_scale = _<Theme0::GameProperties>()
                              .k_worldScaling;
            }
            ...
            void WorldGenerator::GenerateGrass() const
            {
                ...
                {
                    auto xCenter{
                        GetRandomInt(m_size.width)};
                    auto yCenter{
                        GetRandomInt(m_size.height)};
                ...
                int recursion) const
            {
                ...
                auto xCenter{minX +
                             GetRandomInt(maxX - minX)};
                auto yCenter{minY +
                             GetRandomInt(maxY - minY)};
                ...
                GenerateSingleLake(xCenter - maxRadius,
                                   yCenter - maxRadius,
                                   xCenter + maxRadius,
                                   yCenter + maxRadius,
                                   recursion - 1);
            ...
            void WorldGenerator::GenerateRivers() const
            {
                ...
                {
                    auto x{
                        CFloat(GetRandomInt(m_size.width))};
                    auto y{CFloat(
                        GetRandomInt(m_size.height))};
                    ...
                    SharedPtr<Theme0::WorldStructure::Tile>
                        prevTile;
                ...
                for (auto i = 0; i < numRats; i++)
                {
                    ...
                    if (tile && !tile->GetCreature() &&
                        tile->GetGround() !=
                            Hash("ground_water"))
                    {
                        auto newCreature = std::make_shared<
                            Theme0::WorldStructure::
                ...
                for (auto i = 0; i < numNPC0s; i++)
                {
                    ...
                    if (tile && !tile->GetNPC() &&
                        tile->GetGround() !=
                            Hash("ground_water"))
                    {
                        auto newNPC = std::make_shared<
                            Theme0::WorldStructure::NPC>(
                            "npc0");

                        tile->SetNPC(newNPC);

                        m_worldArea->GetNPCsMirrorRef()
                            .insert(
                                {tile->GetNPC(), {x, y}});
                    }
                }
