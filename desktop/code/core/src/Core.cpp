    ...
    namespace Core
    {
        ...
        SharedPtr<SDL_Window> Engine::SDLDevice::CreateWindow()
        {
            ...
            auto windowResult{SharedPtr<SDL_Window>(
                SDL_CreateWindow(m_gameWindowTitle.data(),
                                 ...
                                 screenSize.width, screenSize.height, flags),
                SDLDeleter())};
        ...
        void Engine::Cursor::Render()
        {
            ...
            auto height{ConvertWidthToHeight(
                k_cursorSize, _<Engine::SDLDevice>().GetWindow())};
            ...
            for (auto it : rdi)
            {
                ...
                if (GetFileExtension(filePath) == "png")
                {
                    ...
                    auto surface{SharedPtr<SDL_Surface>(
                        IMG_Load(filePath.data()), SDLDeleter())};
        ...
        GLuint
        Engine::Assets::Images::ImageBank::ObtainTextTexture(float x, float y,
                                                             int textHash)
        {
        ...
        void Engine::Assets::Models::ModelBank::Model::Initialize(
            StringView filePath)
        {
            ...
            aiNode *node, const aiScene *scene, aiMatrix4x4 transformation)
        {
            for (unsigned int i = 0; i < node->mNumMeshes; i++)
            {
                ...
                // Only apply transformation on meshes not entities such as
                // lights or camera.
        ...
        bool
        Engine::Input::KeyboardInput::KeyIsPressedPickResult(SDL_Keycode key)
        {
