...
namespace Forradia
{
    ...
    void RenderersCollection::Image2DRenderer::DrawImage(int imageNameHash,
                                                         float x, float y,
                                                         float width,
                                                         float height)
    {
    ...
    void RenderersCollection::Image2DRenderer::DrawTexture(
        GLuint textureID, float x, float y, float width, float height,
        bool useOperationsMemory)
    {
        ...
        if (needFillBuffers)
        {
            ...
            glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                         sizeof(indices[0]) * indicesCount, indices,
                         GL_DYNAMIC_DRAW);

            glBindBuffer(GL_ARRAY_BUFFER, vbo);

            glBufferData(GL_ARRAY_BUFFER,
                         sizeof(vertices[0]) * 8 * verticesCount, vertices,
                         GL_DYNAMIC_DRAW);
    ...
    void RenderersCollection::GroundRenderer::DrawTile(
        ...
        Point3F cameraPosition, Vector<float> &elevations,
        ...
        GLuint textureID, Vector<float> &verticesVec, Point3F cameraPosition)
    {
        ...
        else
        {
            ...
            m_operationsMemory[verticesVec.at(0)][verticesVec.at(1)]
                              [textureID] = entry;
        ...
        if (needFillBuffers)
        {
            ...
            glBufferData(GL_ARRAY_BUFFER,
                         sizeof(vertices[0]) * 11 * verticesCount, vertices,
                         GL_DYNAMIC_DRAW);
        ...
        modelMatrix =
            glm::translate(modelMatrix, glm::vec3(-0.5f, -0.5f, 0.0f));
    ...
    void RenderersCollection::ModelRenderer::DrawModel(int modelNameHash,
                                                       float x, float y,
                                                       float elevation,
                                                       ...
                                                       float elevationHeight)
    {
        ...
        if (m_operationsMemory.contains(x) &&
            ...
            m_operationsMemory.at(x).at(y).at(elevation).contains(
                modelNameHash))
        {
            needCreateBuffers = false;
        }
        ...
        if (needFillBuffers)
        {
            ...
            for (auto &mesh : meshes)
            {
                ...
                {
                    verticesVector.push_back(x +
                                             vertex.position.x * k_modelScale);
                    verticesVector.push_back(y +
        ...
        glDrawElements(GL_TRIANGLES, entry.verticesCount, GL_UNSIGNED_INT,
                       nullptr);
    ...
    void RenderersCollection::TextRenderer::AddFonts()
    {
        ...
        for (auto fontSize : {FontSizes::_20, FontSizes::_26})
        {
            ...
            auto newFont{SharedPtr<TTF_Font>(
                TTF_OpenFont(fontPathUnixStyle.c_str(), fontSizeN),
                SDLDeleter())};
                                                       ...
                                                       Color textColor) const
    {
        ...
        auto texture{
            _<Core::Engine::Assets::Images::ImageBank>().ObtainTextTexture(
                xx, yy, textHash)};
        ...
        if (!textureAlreadyExists)
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, 4, intermediary->w, intermediary->h,
                         0, GL_RGBA, GL_UNSIGNED_BYTE, intermediary->pixels);
