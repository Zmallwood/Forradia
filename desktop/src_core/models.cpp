...
mesh model::process_mesh(aiMesh *mesh, const aiScene *scene,
  ...
  return forr::mesh(vertices, indices, textures, extents, origin, mesh->mName);
}
...
vec<texture> model::get_textures(aiMesh *mesh, const aiScene *scene) {
  ...
  //        scene->mTextures std::cout << "embedded texture" << std::endl; auto
  //        mat{scene->mMaterials[atoi(&path.data[1])]};
