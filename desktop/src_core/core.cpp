...
namespace Core {
  ...
  void engine::cursor::init() { disable_sys_curs(); }

  void engine::cursor::disable_sys_curs() { SDL_ShowCursor(SDL_DISABLE); }
  ...
  void engine::Assets::Images::image_bank::init() { load_imgs(); }
  ...
  s_ptr<SDL_Texture>
  engine::Assets::Images::image_bank::get_img(int img_name_hash) const {
  ...
  s_ptr<SDL_Texture>
  engine::Assets::Images::image_bank::load_single_img(s_ptr<SDL_Surface> surf) {
    // if (surf) {
  ...
  void engine::Assets::Models::model_bank::model::process_node(aiNode *node,
                                                   const aiScene *scene,
  ...
  engine::Assets::Models::model_bank::mesh engine::Assets::Models::model_bank::model::process_mesh(aiMesh *mesh,
                                                   const aiScene *scene,
                                                   aiMatrix4x4 transformation) {
    glm::vec3 extents;
    glm::vec3 origin;
    ...
    return engine::Assets::Models::model_bank::mesh(vertices, indices, textures, extents, origin,
                      mesh->mName);
  }

  vec<engine::Assets::Models::model_bank::vertex>
  engine::Assets::Models::model_bank::model::get_vertices(aiMesh *mesh, glm::vec3 &extents,
                                              glm::vec3 &origin,
                                              aiMatrix4x4 transformation) {
    vec<vertex> vertices;
  ...
  vec<unsigned int> engine::Assets::Models::model_bank::model::get_indices(aiMesh *mesh) {
  ...
  engine::Assets::Models::model_bank::model::get_textures(aiMesh *mesh,
                                              const aiScene *scene) {
    ...
    //      if ('*' == path.data[0]) {
    //        // embedded texture, get index from string and access
    //        scene->mTextures std::cout << "embedded texture" << std::endl;
