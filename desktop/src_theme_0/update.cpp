...
void update_npcs() {
  ...
  for (auto it = npcs.begin(); it != npcs.end();) {
    ...
    if (now > npc->ticks_next_spontaneous_speech()) {
      auto name{npc->name()};
      if (rand_int(20) == 0)
        _<gui_chat_box>().print(name + ": Buying blueberries, one gold each.");
      else
...
void tl_hovering::update() {
  ...
  for (auto y = -extra_rows; y < grid_sz.h + extra_rows; y++) {
    ...
    if (tl_nw->elev() > tl_sw->elev() && tl_ne->elev() > tl_se->elev())
      ...
      local_tl_h = tl_sz.h * 1.0f;
    else if (tl_sw->elev() == tl_ne->elev() && tl_nw->elev() < tl_sw->elev() &&
             tl_se->elev() < tl_sw->elev())
      local_tl_h = tl_sz.h * 1.5f;
    else if (tl_nw->elev() == tl_se->elev() && tl_ne->elev() < tl_nw->elev() &&
             tl_sw->elev() < tl_nw->elev())
      local_tl_h = tl_sz.h * 1.5f;
    else
      local_tl_h = tl_sz.h;
    auto rect{rect_f{screen_rel_x_px, screen_rel_y_px - local_tl_h / 2, tl_sz.w,
                     local_tl_h}};
