...
void world_view::render() const
{
    ...
    static func<bool()> fn_draw_regular_ground{
        [&]
        {
            ...
            _<img_2d_rend>().draw_img(ground_type_rend, x_canv, y_canv, w_canv,
                                      h_canv);
            return true;
        }};
            ...
            for (auto j = 0; j < 2; j++)
            {
                ...
                case dirs::n:
                    for (auto k = 0.0f; k < part_len; k += 0.5f)
                    {
                        ...
                        _<img_2d_rend>().draw_img("river_part_" +
    ...
    static func<void()> fn_draw_water{
        ...
        {
            auto anim_idx{(ticks() + ((x_coord + y_coord) * 100)) / 500 % 3};
            _<img_2d_rend>().draw_img("ground_water_" +
                                          std::to_string(anim_idx),
                                      x_canv, y_canv, w_canv, h_canv);

            for (auto i = 0; i < tl->elev(); i++)
                _<img_2d_rend>().draw_img("ground_water_height", x_canv, y_canv,
                                          w_canv, h_canv);
            for (auto i = 0; i < tl->water_depth(); i++)
                _<img_2d_rend>().draw_img("ground_water_depth", x_canv, y_canv,
                                          w_canv, h_canv);
        }};
    static func<void()> fn_draw_layers{
        ...
        {
            if (ground == hash("ground_grass"))
                _<img_2d_rend>().draw_img("ground_grass_layer", x_canv, y_canv,
                                          w_canv, h_canv);
            else if (ground == hash("ground_dirt"))
                _<img_2d_rend>().draw_img("ground_dirt_layer", x_canv, y_canv,
                                          w_canv, h_canv);
            else if (ground == hash("ground_rock"))
                _<img_2d_rend>().draw_img("ground_rock_layer", x_canv, y_canv,
                                          w_canv, h_canv);
        }};
    static func<void()> fn_draw_tile_symbols{
        []
        {
            ...
            if (x_coord == hovered_coord.x && y_coord == hovered_coord.y)
                _<img_2d_rend>().draw_img("hovered_tile", x_canv, y_canv,
                                          w_canv, h_canv);
    ...
    static func<void()> fn_draw_player{
        [&]
        {
            ...
            {
                _<img_2d_rend>().draw_img("shadow", x_canv, y_canv, w_canv,
                                          h_canv);
                _<img_2d_rend>().draw_img("player", x_canv, y_canv, tl_sz.w,
                                          tl_sz.h);
            }
        }};
            ...
            if (objs_stack)
            {
                for (const auto &obj : objs_stack->objects())
                {
                    if (obj->type() != hash("object_pink_flower"))
                        _<img_2d_rend>().draw_img("shadow", x_canv, y_canv,
                                                  tl_sz.w, tl_sz.h);
        ...
        for (auto x = 0; x < grid_sz.w; x++)
        {
                ...
                y_coord == wa_sz.h - player_pos.y)
            {
                _<model_rend>().draw_model(
                    ...
                    elev_h);
                _<ground_rend>().draw_tile(hash("ground_dirt"), x_coord,
