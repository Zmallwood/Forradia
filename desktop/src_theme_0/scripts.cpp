...
namespace forr {
  ...
  PYBIND11_EMBEDDED_MODULE(embedded, m) {
    ...
    py::class_<gui, s_ptr<gui>>(m, "gui").def(
        "add_child_comp",
        [](gui &self, s_ptr<gui_comp> comp) -> s_ptr<gui_comp> {
    ...
    py::class_<img_2d_rend>(m, "img_2d_rend")
        ...
        .def("draw_img",
             [](img_2d_rend &self, int image_name_hash, float x, float y,
                float w,
                float h) { self.draw_img(image_name_hash, x, y, w, h); })
        .def("draw_img_auto_h",
             [](img_2d_rend &self, str_view img_name, float x, float y,
                float w) { self.draw_img_auto_h(img_name, x, y, w); });
    ...
    m.def(
        "get_gui_chat_box",
        []() -> gui_chat_box & { return _<gui_chat_box>(); },
        py::return_value_policy::reference);
