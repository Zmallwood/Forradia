    ...
    for (auto i = 0; i < 3; i++) {
      for (auto y = -extraRows; y < gridSize.height + extraRows; y++) {
        for (auto x = 0; x < gridSize.width; x++) {
          ...
          if (i == 0) {
            _<ImageRenderer>().DrawImage(groundTypeRendered, xCanvas, yCanvas,
            ...
            for (auto j = 0; j < 2; j++) {
              ...
              case Directions::North: {
                for (auto k = 0.0f; k < partLenght; k += 0.5f) {
                  RectF bounds{xCanvas + widthCanvas / 2 - riverPartWidth / 2,
            ...
            for (auto i = 0; i < tile->GetElevation(); i++) {
              _<ImageRenderer>().DrawImage("GroundWaterHeight", xCanvas,
            ...
            if (ground == Hash("GroundWater")) {
              for (auto i = 0; i < tile->GetWaterDepth(); i++) {
                _<ImageRenderer>().DrawImage("GroundWaterDepth", xCanvas,
                                             yCanvas, widthCanvas,
                                             heightCanvas);
              }
            }
          ...
          if (ground == Hash("GroundGrass")) {
            _<ImageRenderer>().DrawImage("GroundGrassLayer", xCanvas, yCanvas,
                                         widthCanvas, heightCanvas);
          } else if (ground == Hash("GroundDirt")) {
            _<ImageRenderer>().DrawImage("GroundDirtLayer", xCanvas, yCanvas,
                                         widthCanvas, heightCanvas);
          } else if (ground == Hash("GroundRock")) {
            _<ImageRenderer>().DrawImage("GroundRockLayer", xCanvas, yCanvas,
                                         widthCanvas, heightCanvas);
          }
          ...
          if (xCoordinate == hoveredCoordinate.x &&
            ...
            _<ImageRenderer>().DrawImage("HoveredTile", xCanvas, yCanvas,
                                         widthCanvas, heightCanvas);
          }
          ...
          if (xCoordinate == playerDestination.x &&
            ...
            _<ImageRenderer>().DrawImage("DestinationTile", xCanvas, yCanvas,
                                         widthCanvas, heightCanvas);
          }

          if (xCoordinate == playerPosition.x &&
              yCoordinate == playerPosition.y) {
            _<ImageRenderer>().DrawImage("Shadow", xCanvas, yCanvas,
                                         widthCanvas, heightCanvas);
            _<ImageRenderer>().DrawImage("Player", xCanvas, yCanvas,
          ...
          if (objectsStack) {
            for (const auto &object : objectsStack->GetObjects()) {
              ...
              auto objectImageSize{_<ImageBank>().GetImageSize(objectType)};
          ...
          if (creature) {
            ...
            auto creatureImageSize{_<ImageBank>().GetImageSize(creatureType)};
