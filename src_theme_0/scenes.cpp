...
namespace forr {
  void intro_scene::setup_funcs() {
    set_init_derived([this] {
      start_text_comp_ = gui()->add_child_comp(std::make_shared<gui_label>(
          0.45f, 0.5f, 0.1f, 0.04f, "Press to start", true));
    });
    set_on_enter_derived([] { _<gui_chat_box>().print("Game started."); });
    set_update_derived([this] {
  ...
  void world_gen_scene::setup_funcs() {
  ...
  void main_scene::setup_funcs() {
    ...
    set_render_derived([] { _<world_view>().render(); });
  }
}