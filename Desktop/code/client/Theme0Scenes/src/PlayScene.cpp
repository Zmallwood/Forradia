...
namespace Forradia::Theme0 {
  auto PlayScene::InitializeDerived() -> void {
    ...
    auto btnNewGame{
        std::make_shared<GUIButton>("PlaySceneButtonNewGame", 0.45, 0.36, 0.1, 0.04, "New game",
    ...
    auto btnBack{std::make_shared<GUIButton>("PlaySceneButtonBack", 0.45f, 0.56f, 0.1f, 0.04f,
                                             "Back",
  ...
  auto PlayScene::RenderDerived() const -> void {
    _<Image2DRenderer>().DrawImageByName(Hash("PlaySceneBackground"), "DefaultSceneBackground",
                                         0.0f, 0.0f, 1.0f, 1.0f);
    _<Image2DRenderer>().DrawImageAutoHeight(Hash("PlaySceneLogo"), "ForradiaLogo", 0.35f, 0.1f,
                                             0.3f);
  }
}
