...
void GUIPlayerBodyWindow::Initialize() {
  ...
  m_labelBodyPartName = std::make_shared<GUILabel>("GUILabelBodyPartName", 0.01f, 0.3f, 0.2f, 0.05f,
                                                   "Body part: ", false,
                                                   Palette::GetColor<Hash("YellowTransparent")>());
  m_labelBodyPartStrength = std::make_shared<GUILabel>("GUILabelBodyPartStrength", 0.01f + 0.01f,
                                                       0.3f + 1 * 0.02f, 0.2f, 0.05f, "Strength: ");
  m_labelBodyPartEnergy = std::make_shared<GUILabel>("GUILabelBodyPartEnergy", 0.01f + 0.01f,
                                                     0.3f + 2 * 0.02f, 0.2f, 0.05f, "Energy: ");
  m_labelBodyPartTemperature = std::make_shared<GUILabel>(
      "GUILabelBodyPartTemperature", 0.01f + 0.01f, 0.3f + 3 * 0.02f, 0.2f, 0.05f, "Temperature: ");
...
void GUIPlayerBodyWindow::UpdateBodyPartInfoLabels() {
  ...
  case overallBody: {
    ...
    auto maxEnergy{
        playerBody.GetBodyPartPtr(Theme0::PlayerBodyPartTypes::OverallBody)->GetMaxEnergy()};

    auto currentEnergy{
        playerBody.GetBodyPartPtr(Theme0::PlayerBodyPartTypes::OverallBody)->GetCurrentEnergy()};

    m_labelBodyPartEnergy->SetText(
        fmt::format("Energy: {:.2f} / {:.2f}", currentEnergy, maxEnergy));

    auto temperature{
        playerBody.GetBodyPartPtr(Theme0::PlayerBodyPartTypes::OverallBody)->GetTemperature()};
  ...
  case rightArm: {
    ...
    auto strength{playerBody.GetBodyPartPtr(Theme0::PlayerBodyPartTypes::RightArm)->GetStrength()};
    ...
    auto currentEnergy{
        playerBody.GetBodyPartPtr(Theme0::PlayerBodyPartTypes::RightArm)->GetCurrentEnergy()};
  ...
  case leftArm: {
    ...
    auto strength{playerBody.GetBodyPartPtr(Theme0::PlayerBodyPartTypes::LeftArm)->GetStrength()};

    m_labelBodyPartStrength->SetText(fmt::format("Strength: {:.2f}", strength));

    auto maxEnergy{playerBody.GetBodyPartPtr(Theme0::PlayerBodyPartTypes::LeftArm)->GetMaxEnergy()};

    auto currentEnergy{
        playerBody.GetBodyPartPtr(Theme0::PlayerBodyPartTypes::LeftArm)->GetCurrentEnergy()};
  ...
  case legs: {
    ...
    auto strength{playerBody.GetBodyPartPtr(Theme0::PlayerBodyPartTypes::Legs)->GetStrength()};

    m_labelBodyPartStrength->SetText(fmt::format("Strength: {:.2f}", strength));

    auto maxEnergy{playerBody.GetBodyPartPtr(Theme0::PlayerBodyPartTypes::Legs)->GetMaxEnergy()};
