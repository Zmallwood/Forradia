...
namespace AAK
{
    namespace Forradia::Theme0
    {
        ...
        void GUIInteractionMenu::BuildMenu()
        {
            ...
            m_clickedRobot =
                worldArea->GetTile(hoveredCoordinate.x, hoveredCoordinate.y)->GetRobot();
            ...
            {
                m_entries.push_back(
                    {"Battle robot", GetAction<Hash("ActionTargetRobot")>().action});
            }
        }
        ...
        void GUIInteractionMenu::RenderDerived() const
        {
            ...
            for (auto &entry : m_entries)
            {
                _<TextRenderer>().DrawString(m_renderIDsMenuEntryStrings[i], entry.GetLabel(),
