...
void WorldView::Render() const {
  ...
  auto groundGridSize{decltype(gridSize){
      static_cast<decltype(gridSize.width)>(gridSize.width * k_groundRenderingDistanceMultiplier),
  ...
  auto fnIterationGround{[&](int x, int y) {
    ...
    tiles.push_back({m_renderIDsGround.at(xCoordinate).at(yCoordinate), ground, xCoordinate,
                     yCoordinate, rendTileSize, elevations, forceRedraw, color00, color10, color11,
                     color01});
  }};
  ...
  auto fnIterationAllExceptGround{[&](int x, int y) {
    ...
    if (isWithinNormalGrid) {
      for (auto object : objects) {
        ...
        _<ModelRenderer>().DrawModel(objectType, (xCoordinate)*rendTileSize + rendTileSize / 2,
